...
// generated surface under tools/_preview (emitted by tools/codegen.py
...
func scanDir(t *testing.T, dir string, syms map[string]bool) int {
	...
	if err != nil {
		return 0 // optional source (e.g. tools/_preview absent)
	}
	files := 0
...
// exposedSymbols = hand-written root *.go ∪ IDL-driven tools/_preview/*.go.
func exposedSymbols(t *testing.T, repo string) map[string]bool {
	t.Helper()
	syms := map[string]bool{}
	scanDir(t, repo, syms)
	scanDir(t, filepath.Join(repo, "tools", "_preview"), syms)
	if len(syms) == 0 {
...
// IDL-driven tools/_preview) must be a superset of
